...
void find_test() {
	...
	if (loc != std::end(vec))
		std::cout << "Found the numer: " << *loc << std::endl;
	else
...
void count_test() {
	...
	int num = std::count(std::begin(vec), std::end(vec), 1);
...
void count_if_test() {
	...
	std::cout << num << " numbers that are >= 5" << std::endl;
}
...
void string_transform_test() {
	...
	std::cout << "Before Transformation: " << str1 << std::endl;
	std::transform(str1.begin(), str1.end(), str1.begin(), std::toupper);
	std::cout << "After Transformation: " << str1 << std::endl;
}
